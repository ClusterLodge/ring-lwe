...
pub fn test_hom_add() {
    ...
    let pk0_fwd = Fwd::<u32>::new(pk[0].coeffs(), q, &params.ntt_plan);
    let pk1_fwd = Fwd::<u32>::new(pk[1].coeffs(), q, &params.ntt_plan);
    let u = encrypt(
        pk0_fwd.clone(),
        pk1_fwd.clone(),
        &m0_poly,
    ...
    let decrypted_sum = decrypt(
        Fwd::<u32>::new(&sk.coeffs(), q, &params.ntt_plan),
        &ciphertext_sum,
        &params,
    );
...
pub fn test_hom_prod() {
    ...
    let pk0_fwd = Fwd::<u32>::new(pk[0].coeffs(), q, ntt_plan);
    let pk1_fwd = Fwd::<u32>::new(pk[1].coeffs(), q, ntt_plan);
